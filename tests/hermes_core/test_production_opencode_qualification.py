"""EA-4E.13 pre-live negative gate tests."""

from __future__ import annotations

from tools.hermes_core.production_opencode_qualification import ProductionOpenCodeQualificationHarness
...
def test_missing_activation():
    harness = ProductionOpenCodeQualificationHarness()
    auth = _build_auth("opencode-cli-agent", "gate1")
    r = harness.dispatch(receiver_id="opencode-cli-agent", authority=auth, activation=None)
...
def test_disabled_activation():
    harness = ProductionOpenCodeQualificationHarness()
    auth = _build_auth("opencode-cli-agent", "gate2")
    act = _build_activation("opencode-cli-agent", "DISABLED")
    r = harness.dispatch(receiver_id="opencode-cli-agent", authority=auth, activation=act)
    assert r.dispatch_decision == "REJECT"
...
def test_missing_authority():
    harness = ProductionOpenCodeQualificationHarness()
    act = _build_activation("opencode-cli-agent", "ENABLED")
    r = harness.dispatch(receiver_id="opencode-cli-agent", authority=None, activation=act)
    assert r.dispatch_decision == "REJECT"
...
def test_denied_authority():
    ...
    denied_auth = build_dispatch_authority(
        ...
        router_contract_id=compute_ea4e6_router_contract_id(),
        scope=DispatchAuthorityScope(operation="receiver-dispatch", receiver_id="opencode-cli-agent"),
        delegation_class="governed",
        decision="DENIED",
        issued_at="2026-01-01T00:00:00Z",
        expires_at="2099-01-01T00:00:00Z",
    ...
    act = _build_activation("opencode-cli-agent", "ENABLED")
    r = harness.dispatch(receiver_id="opencode-cli-agent", authority=denied_auth, activation=act)
    assert r.dispatch_decision == "REJECT"
    assert r.reason == "EXECUTION_AUTHORITY_DENIED"
    assert harness.invocation_count == 0


def test_authority_receiver_mismatch():
    harness = ProductionOpenCodeQualificationHarness()
    wrong_auth = _build_auth("kilo-cli-agent", "gate5")
    act = _build_activation("opencode-cli-agent", "ENABLED")
    r = harness.dispatch(receiver_id="opencode-cli-agent", authority=wrong_auth, activation=act)
    assert r.dispatch_decision == "REJECT"
...
def test_activation_receiver_mismatch():
    harness = ProductionOpenCodeQualificationHarness()
    auth = _build_auth("opencode-cli-agent", "gate6")
    wrong_act = _build_activation("kilo-cli-agent", "ENABLED")
    r = harness.dispatch(receiver_id="opencode-cli-agent", authority=auth, activation=wrong_act)
    assert r.dispatch_decision == "REJECT"
...
def test_router_contract_mismatch():
    harness = ProductionOpenCodeQualificationHarness()
    auth = _build_auth("opencode-cli-agent", "gate7")
    act = _build_activation("opencode-cli-agent", "ENABLED", router_contract_id="wrong")
    r = harness.dispatch(receiver_id="opencode-cli-agent", authority=auth, activation=act)
    assert r.dispatch_decision == "REJECT"
...
def test_authority_contract_mismatch():
    harness = ProductionOpenCodeQualificationHarness()
    auth = _build_auth("opencode-cli-agent", "gate8")
    act = _build_activation("opencode-cli-agent", "ENABLED", authority_contract_id="wrong")
    r = harness.dispatch(receiver_id="opencode-cli-agent", authority=auth, activation=act)
    assert r.dispatch_decision == "REJECT"
...
def test_transport_mismatch():
    harness = ProductionOpenCodeQualificationHarness()
    auth = _build_auth("opencode-cli-agent", "gate9")
    act = _build_activation("opencode-cli-agent", "ENABLED", transport_contract_id="wrong")
    r = harness.dispatch(receiver_id="opencode-cli-agent", authority=auth, activation=act)
    assert r.dispatch_decision == "REJECT"
...
def test_model_binding_mismatch():
    harness = ProductionOpenCodeQualificationHarness()
    auth = _build_auth("opencode-cli-agent", "gate10")
    act = _build_activation("opencode-cli-agent", "ENABLED", model_binding_id="wrong")
    r = harness.dispatch(receiver_id="opencode-cli-agent", authority=auth, activation=act)
    assert r.dispatch_decision == "REJECT"
...
def test_no_fallback():
    harness = ProductionOpenCodeQualificationHarness()
    r = harness.dispatch(receiver_id="opencode-cli-agent", authority=None, activation=None)
    assert r.dispatch_decision == "REJECT"
    assert harness.invocation_count == 0
