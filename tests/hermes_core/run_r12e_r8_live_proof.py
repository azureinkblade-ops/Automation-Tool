"""Prepare and execute the one authorized EA-4D.4F-R12E-R8 Codex proof.
...
RUNTIME_OWNER = RuntimeNamespaceOwner("ea4d4f-r12e-r8")
...
SOURCE_FILES = (
    ...
    ROOT / "tests" / "hermes_core" / "run_r12e_r8_live_proof.py",
...
def prepare() -> dict:
    ...
    envelope = build_delegated_task_envelope(
        ...
        originator_request_id="ea4d4f-r12e-r8-one-shot",
        task_id="task-ea4d4f-r12e-r8",
        parent_task_id=None,
        originator_agent_id=ORIGINATOR,
        requested_target_agent_id=RECEIVER,
        operation="return_bounded_qualification_result",
        objective="Return the exact deterministic R12E-R8 qualification result.",
    ...
    delegation.claim_message(
        ...
        claim_token="r12e-r8-receiver-claim",
    ...
    adapter = CodexReceiverAdapter(config=_trusted_config(envelope.task_input_hash, receipt.artifact_hash))
...
def refreeze() -> dict:
    ...
    config = _trusted_config(packet["task_input_hash"], receipt.artifact_hash)
    adapter = CodexReceiverAdapter(config=config)
...
def execute(expected_preflight_hash: str) -> dict:
    ...
    try:
        ...
        originator.claim_message(
            ...
            claim_token="r12e-r8-originator-result-claim",
            claimed_at=_now(), claim_expires_at=lease_window.window.expires_at,
        )
        originator.acknowledge_message(
            result_message_id, recipient_agent_id=ORIGINATOR,
            claim_token="r12e-r8-originator-result-claim", acknowledged_at=_now(),
        )
        evidence.update({
            "completed_at": _now(),
