...
def test_candidate_contract_diff_is_isolated_and_deterministic(monkeypatch):
    ...
    def replace_aliases(old, new):
        for name, module in tuple(sys.modules.items()):
            ...
            for attribute, value in tuple(vars(module).items()):
                if not attribute.startswith(("HISTORICAL", "LEGACY")) and isinstance(value, str) and value == old:
    ...
    def replace_transport(old, new):
        ...
        for name, module in tuple(sys.modules.items()):
            ...
            if isinstance(receivers, dict) and "kilo-cli-agent" in receivers:
                monkeypatch.setitem(receivers["kilo-cli-agent"], "transport_contract_id", new)

    old_transport = binding.HISTORICAL_KILO_7_5_16_TRANSPORT_CONTRACT_ID
    old_path = binding.HISTORICAL_KILO_7_5_16_PATH
    old_hash = binding.HISTORICAL_KILO_7_5_16_SHA256
    old_version = "7.5.16"
    replace_transport(adapter.KILO_TRANSPORT_CONTRACT_ID, old_transport)
    replace_aliases(adapter.PINNED_KILO_PATH, old_path)
    replace_aliases(adapter.PINNED_KILO_SHA256, old_hash)
    replace_aliases(adapter.PINNED_KILO_VERSION, old_version)
    caches = (
    ...
    for key, attribute in caches:
        replace_aliases(getattr(binding, attribute), binding.HISTORICAL_EA4E_7_5_16_CONTRACT_IDS[key])
    old_ids = current_ids()
    assert old_ids == binding.HISTORICAL_EA4E_7_5_16_CONTRACT_IDS
    ...
    old_material = adapter._canonical_material(old_hash, old_version, binary_path=old_path)
    ...
    new_ids = current_ids()
    print(json.dumps(new_ids, sort_keys=True))
    ...
    assert new_binding != binding.HISTORICAL_KILO_7_5_16_EXECUTABLE_BINDING_ID
